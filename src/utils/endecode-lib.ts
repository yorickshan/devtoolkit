...
const EncodeUtils = (() => {
     ...
     */
    const _uniEncode = function (text) {
     ...
     * @param {Object} text
     */
    const _uniDecode = function (text) {
     ...
     * @return {String}
     */
    const _utf16to8 = function (str) {
        return str.replace(/\\x/g, '%');
    };

    const _utf8to16 = function (str) {
    ...
    const hexTools = (function (input) {
        const utf8encode = function (str, isGetBytes) {
            const back = [];
            let byteSize = 0;
     ...
     * @returns {string}
     */
    const _html2js = function (txt) {
        const htmlArr = txt.replace(/\\/g, "\\\\").replace(/\\/g, "\\/").replace(/\'/g, "\\\'").split('\n');
     ...
     * @private
     */
    const _urlParamsDecode = function (url) {
        let res = {};
        try {
            const params = [];
    ...
    // sha1加密
    const _sha1Encode = function (str) {
     ...
     */
    const formatCookieStringToJson = (cookieString) => {
     ...
     */
    const _gzipEncode = async function(text) {
        try {
     ...
     */
    const _gzipDecode = async function(compressedBase64) {
        try {
    ...
    const _stringEscape = function(text) {
        return text
            .replace(/\\/g, '\\\\')
            .replace(/\n/g, '\\n')
            .replace(/\r/g, '\\r')
    ...
    const _stringUnescape = function(text) {
