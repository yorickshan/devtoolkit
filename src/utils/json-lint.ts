    ...
    const lexer = (function () {
        const lexer = ({
            ...
            upcomingInput: function () {
                ...
                const s = (next.substr(0, 20) + (next.length > 20 ? '...' : '')).replace(/\n/g, "");
                return s;
            },
 ...
 * @returns {{}}
 */
const lintDetect = function (sJson) {
    const result = {};

    const insertErrorFlag = function (s) {
        s = s.replace(/^(\s*){([^\s])/, ($0, $1, $2) => ($1 + '{ ' + $2));
        s = s.replace(/([\s,]+)([^,:\{\}\[\]\s'"]+)(\s*:)/gm, ($0, $1, $2, $3) => ($1 + '"' + $2 + '"' + $3));
        s = s.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
        const aLine = s.split('\n');
        jsonlint.yy.parseError = function (sError, oError) {
            const theLineNum = oError.line === oError.loc.first_line ? oError.line - 1 : oError.line;
            const sLine = aLine[theLineNum];
    ...
    sJson = escape(sJson);
    const placeholder = '<span class="x-ph"><<<<</span>';
