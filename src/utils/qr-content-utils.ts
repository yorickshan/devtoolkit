...
function formatEventQrContent(fields, now = new Date()) {
    ...
    const lines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//DevToolkit//QR Code//CN',
        'BEGIN:VEVENT',
        `UID:${encodeURIComponent(fields.title)}-${start.value}@devtoolkit.local`,
...
function parseCoordinatePair(text) {
    ...
    const match = String(text || '').match(new RegExp(`(?:${pairLabel})\\s*(?:=|:|：|是|为)\\s*(-?\\d+(?:\\.\\d+)?)\\s*[,，]\\s*(-?\\d+(?:\\.\\d+)?)`, 'i'));
